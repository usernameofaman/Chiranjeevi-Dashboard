...
`;
const PrintByCheque = styled.div`
  display: flex;
  @media print {
    display: none;
  }
...
function IPDReciept(props) {
  ...
  const handleInputData = (e) => {
    if (e.target.name === "amount" && !isNaN(e.target.value)) {
      let response = FirebaseFunction.toTitleCase(
        FirebaseFunction.inWords(e.target.value)
      );
      let parsedAmount = parseInt(e.target.value);
      setReceipt({
        ...receipt,
        [e.target.name]: parsedAmount,
        amountInWords: response,
      });
    } else {
  ...
  const SaveReceipt = () => {
    ////console.log(patient)
    if (!receipt.serialNo || receipt.serialNo === "") {
      Toast.apiFailureToast("Please get serial no");
      return;
    }
    if (receipt.amount === "") {
    ...
    <>
      <Container>
        <ButtonContainer>
          ...
          <Button onClick={getSerialNo} className={classes.button}>
            {" "}
            Get Serial No{" "}
          </Button>
        </ButtonContainer>
            ...
            <Form>
                ...
                <LogoAndHeading>
                  ...
                  <TextHolder>
                    ...
                    <Typography variant="h6">
                      <b>
                        Virat Sagar Parisar,Oppo. SATI College, NH-86,Vidisha
                        (M.P.)
                      </b>
                      <br />
                      <b>
                        <TelePhone src="/Images/telephone.png" /> : 250544,
                        251280
                      </b>
                    </Typography>
                  </TextHolder>
              ...
              <HeaderInput>
                <div style={{ paddingBottom: "6px" }}>
                  Received with thanks from {patient.sex === "M" ? "Mr" : "Mrs"}{" "}
                </div>
                <Value>
                  <TextField
                    className={classes.inputReceipt}
                    name="name"
                    type="text"
                    size="small"
                    label={!printMode && "Name"}
                    value={patient.name}
                    InputLabelProps={{ shrink: patient.name ? true : false }}
                    disabled
                  />
                </Value>
              </HeaderInput>
              <HeaderInput>
                ...
                <Value>
                  <TextField
                    ...
                    InputLabelProps={{
                      shrink: receipt.amountInWords === "" ? false : true,
                    }}
                    disabled
                  />
                </Value>
              ...
              <HeaderInput style={{ fontSize: "15px" }}>
                <div style={{ paddingBottom: "6px" }}>
                  For Hospitalization / Pathalogical Investigation / OPD / X-Ray
                  / ECG and Other{" "}
                </div>
                <Value>
